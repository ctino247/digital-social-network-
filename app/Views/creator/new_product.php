...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="bg-surface-container-low p-6 rounded-3xl border border-white/5 shadow-xl">
        <h2 class="text-xl font-bold font-geist text-on-background mb-4">Upload New Digital Offering</h2>

        <form action="/creator/products/new" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" required placeholder="e.g. Photoshop Brush Templates" class="w-full px-4 py-2.5 bg-background border border-white/10 rounded-2xl text-on-background focus:border-primary outline-none text-xs font-medium"/>
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" id="price" name="price" required placeholder="19.99" class="w-full px-4 py-2.5 bg-background border border-white/10 rounded-2xl text-on-background focus:border-primary outline-none text-xs font-medium"/>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" required class="w-full px-4 py-2.5 bg-background border border-white/10 rounded-2xl text-on-background focus:border-primary outline-none text-xs font-medium">
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= (int)$cat['id'] ?>"><?= Security::e($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select id="type" name="type" required class="w-full px-4 py-2.5 bg-background border border-white/10 rounded-2xl text-on-background focus:border-primary outline-none text-xs font-medium">
                        ...
                        <option value="Course">Course Access Link</option>
                        <option value="Source Code">Source Code Files</option>
                        <option value="Software">Executable Software</option>
                        <option value="E-book">E-book</option>
                        <option value="Audio">Audio Track</option>
                        <option value="Video">Video File</option>
                        <option value="Digital Download">General Download File</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" required placeholder="Describe what people get when they purchase this product. Supporting details, download guidelines, or features list..." class="w-full p-4 bg-background border border-white/10 rounded-2xl text-on-background focus:border-primary outline-none text-xs leading-relaxed resize-none"></textarea>
            </div>

            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-on-surface-variant mb-1.5">Attach Digital File Binaries</label>
                <input type="file" name="product_file" required class="block w-full text-xs text-on-surface-variant file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 file:cursor-pointer"/>
                <p class="text-[10px] text-on-surface-variant mt-1.5">Select the master digital product file (e.g. books, archives, code) to deliver securely to authorized buyers.</p>
            </div>
