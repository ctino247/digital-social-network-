...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-3xl border border-[#E0E6E2] space-y-4 shadow-sm">
        <h1 class="text-xl font-extrabold text-[#004D40]">Followers of @<?= Security::e($profileUser['username']) ?></h1>

        <form action="/profile/<?= Security::e($profileUser['username']) ?>/followers" method="GET" class="relative">
            <span class="material-symbols-outlined absolute left-4 top-3 text-[#004D40] opacity-55">search</span>
            <input type="text" name="q" value="<?= Security::e($search) ?>" placeholder="Search followers by name or username..." class="w-full pl-11 pr-4 py-2.5 bg-[#FBFBF9] border border-[#E0E6E2] rounded-full text-xs text-on-background focus:border-[#004D40] outline-none transition-all font-bold"/>
        </form>
    </div>
    ...
    <div class="space-y-4">
        <?php if (empty($followers)): ?>
            <div class="text-center py-12 bg-white rounded-3xl border border-[#E0E6E2] text-on-surface-variant shadow-sm">
                <span class="material-symbols-outlined text-4xl mb-2 text-[#004D40] opacity-35">group</span>
                <p class="text-xs font-bold text-[#004D40]">No followers found.</p>
            </div>
        <?php else: ?>
