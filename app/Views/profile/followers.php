...
<div class="space-y-6">
    ...
    <div class="bg-surface-container-low p-6 rounded-3xl border border-white/5 space-y-4 shadow-xl">
        <h1 class="text-xl font-bold font-geist text-white">Followers of @<?= Security::e($profileUser['username']) ?></h1>
        ...
        <form action="/profile/<?= Security::e($profileUser['username']) ?>/followers" method="GET" class="relative">
            ...
            <input type="text" name="q" value="<?= Security::e($search) ?>" placeholder="Search followers by name or username..." class="w-full pl-11 pr-4 py-2.5 bg-background border border-white/5 rounded-full text-xs text-on-background focus:border-primary outline-none transition-all"/>
        </form>
    </div>
    ...
    <div class="space-y-4">
        <?php if (empty($followers)): ?>
            <div class="text-center py-12 bg-surface-container-low rounded-3xl border border-white/5 text-on-surface-variant">
                <span class="material-symbols-outlined text-4xl mb-2 opacity-55">group</span>
                <p class="text-sm font-medium">No followers found.</p>
            </div>
        <?php else: ?>
