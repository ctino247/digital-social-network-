-- =========================================================================
-- MIMSHACK DIGITAL SOCIAL COMMERCE PLATFORM - PRODUCTION DATABASE SCHEMA
-- FOR SHARED HOSTING ENVIRONMENT (No CREATE DATABASE / USE statements)
-- =========================================================================
--
-- Instructions for Shared Hosting Upload (e.g., phpMyAdmin / cPanel):
-- 1. Create a database via your cPanel "MySQL Database Wizard".
-- 2. Create a database user and assign them to your database with ALL privileges.
-- 3. Open phpMyAdmin and click on your newly created database in the left sidebar.
-- 4. Go to the "Import" tab at the top.
-- 5. Select this SQL file and click "Go" or "Import".
-- =========================================================================
...
-- -------------------------------------------------------------------------
-- 4. Products Table (Digital Products Only)
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 5. Posts Table (Threads-Style Social Network)
-- -------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `posts` (
    ...
    `referral_code` VARCHAR(50) DEFAULT NULL, -- For Recommendation Links
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- -------------------------------------------------------------------------
-- 14. Referral Links Table (Recommendations)
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 16. Referral Clicks Table (Analytics / Historical Tracks)
-- -------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `withdrawals` (
    ...
    `user_id` INT NOT NULL,
    `amount` DECIMAL(15, 2) NOT NULL,
    `destination` TEXT NOT NULL, -- Destination Bank/PayPal/Transfer details
...
-- -------------------------------------------------------------------------
-- 23. System Settings Table (Key-Value Setup)
-- -------------------------------------------------------------------------
...
-- =========================================================================
-- INITIAL SEEDS & REQUIRED PLATFORM DEFAULTS
-- =========================================================================

-- 1. Default Commission Settings
...
-- 2. Initial System Settings (Config Keys)
INSERT INTO `system_settings` (`key`, `value`) VALUES
('platform_name', 'Mimshack'),
('platform_brand_color', '#004D40'), -- Emerald Green Accent
('maintenance_mode', '0'),
...
-- 3. Pre-configured Administrator Account
-- Username: admin
-- Password: adminpassword (Hashed with password_hash API)
INSERT INTO `users` (`id`, `username`, `email`, `password_hash`, `full_name`, `role`, `is_verified`, `is_sales_partner`)
VALUES (1, 'admin', 'admin@mimshack.com', '$2y$10$1mFsueThA1JNylU.hbld.uIZA.XvEUaIkoA12s4jGIqfxLalmwBCG', 'System Administrator', 'admin', 1, 1)
ON DUPLICATE KEY UPDATE id=id;

-- 4. Create Wallet for the Admin Account
INSERT INTO `wallets` (`user_id`, `balance`, `pending_balance`)
VALUES (1, 1000.00, 0.00)
ON DUPLICATE KEY UPDATE user_id=user_id;

-- 5. Standard Core Marketplace Categories
INSERT INTO `categories` (`id`, `name`, `slug`) VALUES
(1, 'E-books', 'e-books'),
(2, 'Templates & Assets', 'templates'),
(3, 'Software & Code', 'software-code'),
(4, 'Video & Audio Courses', 'video-audio')
ON DUPLICATE KEY UPDATE id=id;
